...
result:
   if ($error === true) {
      ...
      if ($esta_cod === 'ELIMINO') {
         ...
         $Log->RegistraLinea('ADM: SERVICIO DE TRACKER ELIMINADO | BUSUA_COD: ' . ($busua_cod) . ' | TIPO_COD: ' . ($Tipo->tipo_cod) . ' | DESC_TIPO: ' . ($Tipo->tipo_servicio) . ' | CAUSA: ' . ($Tracker->causa));
      } else {
         $Tipo->busca($tipo_cod, $DB);
         $Log->RegistraLinea('ADM: SERVICIO DE TRACKER AGREGADO | BUSUA_COD: ' . ($busua_cod) . ' | TIPO_COD: ' . ($Tipo->tipo_cod) . ' | DESC_TIPO: ' . ($Tipo->tipo_servicio) . ' | CAUSA: ' . ($Tracker->causa));
      }
