<?
   ...
   if ($oper_cod === false || $esta_cod === false) {
   ...
   if ($Operador->actualizaEstado($esta_cod, $DB) === false) {
...
result:
   if ($error === true) {
      ...
      $Log->RegistraLinea('ADM: ESTADO DE OPERADOR ACTUALIZADO | OPER_COD: ' . ($oper_cod) . ' | NUEVO ESTADO: ' . ($esta_cod === 1 ? 'ACTIVO' : 'INACTIVO'));
