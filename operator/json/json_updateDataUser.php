...
result:
   if ($error === true) {
      ...
      $desc = 'DATOS USUARIO BP MODIFICADO | ID CONTACT CENTER: ' . $Usuario->group_cod . ' | BUSUA_COD: ' . $Usuario->busua_cod . ' | CLOUD_USERNAME: ' . $Usuario->cloud_username . ' | NOMBRE: ' . $Usuario->nombre . ' | TELÉFONO USUARIO: ' . $Usuario->user_phone . ' | EMAIL: ' . $Usuario->email;
      $Log->inserta($Operador->oper_cod, $desc, 'operator/json/json_updateDataUser.php', $DB);

      $data = array( 'status'             => 'success',
                     'message'            => 'Datos del usuario <span class="text-primary">Actualizados</span>',
