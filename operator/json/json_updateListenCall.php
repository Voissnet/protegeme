<?
   ...
   if ($check === true) {
      if (intval($Llamada->cantidadEscucha($Usuario->busua_cod, $DB)) > 1 ) {
         $message = 'La plataforma solo permite 2 contactos para escuchar la emergencia';
         $error = true;
         goto result;
      }
   }

   if ($Llamada->actualizaEscucha($Usuario->busua_cod, $numero, $check === true ? 1 : 0, $DB) === false) {
...
result:
   if ($error === true) {
      ...
      $desc = 'ESCUCHAR LLAMADA PARA CONTACTO | BUSUA_COD: ' . $Usuario->busua_cod . ' | NÚMERO CONTACTO: ' . $numero . ' | ESCUCHA: ' . ($check === true ? 'SI' : 'NO');
      $Log->inserta($Operador->oper_cod, $desc, 'operator/json/json_updateListenCall.php', $DB);
